...
namespace EntidadesCentralita
{
    public class Provincial : Llamada, IGuardar<string>
    {
        ...
        public void Guardar()
        {
            if (this.GetType() == typeof(Provincial))
            {
                string path = $"{Environment.CurrentDirectory}\\SerializacionProvincial.xml";
                XmlTextWriter escribirObjeto = new XmlTextWriter(path, System.Text.Encoding.UTF8);
                XmlSerializer serializarObjeto = new XmlSerializer(typeof(Provincial));
